                    ...
                    <tbody>
                        <template v-for="purchase in purchases">
                            <tr>
                                ...
                                <td style="text-align: left;">{{ purchase.purchaseDetails[0].name }}</td>
                                ...
                                <td style="text-align:right;">{{ purchase.purchaseDetails[0].total }}</td>
                                <td style="text-align:center;">
                                    <a href="" title="Purchase Invoice" v-bind:href="`/material_purchase_invoice/${purchase.purchase_id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    <?php if ($this->session->userdata('accountType') != 'u') { ?>
                                        <a href="javascript:" title="Edit Purchase" @click="checkReturnAndEdit(purchase)"><i class="fa fa-edit"></i></a>
                                        <a href="" title="Delete Purchase" @click.prevent="deletePurchase(purchase.purchase_id)"><i class="fa fa-trash"></i></a>
                                    <?php } ?>
                                </td>
                            </tr>
                            ...
                            <tr style="font-weight:bold;">
                                <td colspan="4" style="font-weight:normal;"><strong>Note: </strong>{{ purchase.note }}</td>
                ...
                <table class="table record-table table-hover table-bordered" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
                    ...
                    <tbody>
                        <tr v-for="purchase in purchases">
                            <td>{{ purchase.invoice_no }}</td>
                            <td>{{ purchase.purchase_date }}</td>
                            <td style="text-align: left;">{{ purchase.supplier_name }}</td>
                            <td style="text-align:right;">{{ purchase.sub_total }}</td>
                            <td style="text-align:right;">{{ purchase.vat }}</td>
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.sub_total)}, 0) }}</td>
                            <td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.vat)}, 0) }}</td>
                            <td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.discount)}, 0) }}</td>
    ...
    new Vue({
        ...
        methods: {
            checkReturnAndEdit(purchase) {
                location.replace('/material_purchase/' + purchase.purchase_id);
            },
            onChangeSearchType() {
                this.purchases = [];
            ...
            getPurchaseRecord() {
                ...
                let url = '/get_material_purchase';
                if (this.recordType == 'with_details') {
                    url = '/get_material_purchase';
                }
            ...
            getPurchaseDetails() {
                ...
                axios.post('/get_material_purchase_details', filter)
                    .then(res => {
                        this.purchases = res.data;
                    })
            },
            deletePurchase(purchaseId) {
                ...
                axios.post('/delete_material_purchase', {
