...
class Login extends CI_Controller {
    ...
    function procedure(){
		...
		$query = $this->db->query("SELECT u.User_SlNo, u.User_ID, u.FullName, u.User_Name, u.userBranch_id, u.UserType, u.image_name as user_image, u.status AS userstatus, br.branch_id, br.Branch_name, br.Branch_sales, br.is_production FROM tbl_user AS u LEFT JOIN tbl_branch AS br ON br.branch_id = u.userBranch_id where br.status = 'a' and u.User_Name = ? AND u.User_Password = ?", [$user, $pass]);
		...
		if(isset($data)){
			...
			if ($data->userstatus=='a') {
				...
				$sdata['is_production'] = $data->is_production;
