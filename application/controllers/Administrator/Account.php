    ...
    public function getCashLedger()
    {
        ...
        $ledger = $this->db->query("
            ...
            select 
                ...
                concat('Material Sale - ', sm.invoice_no, ' - ', ifnull(c.Customer_Name, sm.customerName), ' (', ifnull(c.Customer_Code, 'Cash Customer'), ')', ' - Bill: ', sm.total) as description,
            ...
            and sm.sale_date between '$data->fromDate' and '$data->toDate'
            ...
            UNION            
            select 
                pm.purchase_id as id,
                pm.purchase_date as date,
                concat('Material Purchase - ', pm.invoice_no, ' - ', ifnull(s.Supplier_Name, pm.supplierName), ' (', ifnull(s.Supplier_Code, 'Cash Supplier'), ')', ' - Bill: ', pm.total) as description,
                0.00 as in_amount,
                pm.paid as out_amount
            from tbl_material_purchase pm 
            left join tbl_supplier s on s.Supplier_SlNo = pm.Supplier_SlNo
            where pm.status = 'a'
            and pm.branch_id = '$this->brunch'
            and pm.purchase_date between '$data->fromDate' and '$data->toDate'
