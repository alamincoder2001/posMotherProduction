        ...
        $stock = $this->db->query("
            select
                ...
                (select ifnull(sum(pp.quantity), 0)
                        ...
                        and pp.status = 'a'
                    " . (isset($date) && $date != null ? " and pro.date <= '$date'" : "") . "
                    ) as production_quantity,

                    (select ifnull(sum(cpi.quantity), 0)
                        from tbl_convert_product_in cpi
                        left join tbl_convert_master pro on pro.id = cpi.convert_id
                        where cpi.product_id = p.Product_SlNo
                        and cpi.branch_id = p.branch_id
                        and cpi.status = 'a'
                    " . (isset($date) && $date != null ? " and pro.date <= '$date'" : "") . "
                    ) as convert_quantity_in,

                    (select ifnull(sum(cpo.quantity), 0)
                        from tbl_convert_product_out cpo
                        left join tbl_convert_master pro on pro.id = cpo.convert_id
                        where cpo.product_id = p.Product_SlNo
                        and cpo.branch_id = p.branch_id
                        and cpo.status = 'a'
                    " . (isset($date) && $date != null ? " and pro.date <= '$date'" : "") . "
                    ) as convert_quantity_out,
                        
                (select (production_quantity + purchased_quantity + convert_quantity_in + sales_returned_quantity + transferred_to_quantity) - (sold_quantity + convert_quantity_out + purchase_returned_quantity + damaged_quantity + transferred_from_quantity)) as current_quantity,
    ...
    public function getProductLedger()
    {
        ...
        $result = $this->db->query("
            select 
                ...
                p.date,
                concat('Production - ', p.production_sl, ' - ', ifnull(p.note, '')) as description,
                pp.price as rate,
            ...
            select 
                ...
                cpi.id as id,
                cpm.date,
                concat('Convert Product In - ', cpm.invoice) as description,
                cpi.price as rate,
                cpi.quantity as in_quantity,
                0 as out_quantity   
            from tbl_convert_product_in cpi
            join tbl_convert_master cpm on cpm.id = cpi.convert_id
            where cpi.status = 'a'
            and cpi.product_id = " . $data->productId . "
            and cpi.branch_id = " . $this->brunch . "
            ...
            select 
                ...
                cpo.id as id,
                cpm.date,
                concat('Convert Product Out - ', cpm.invoice) as description,
                cpo.price as rate,
                0 as in_quantity,
                cpo.quantity as out_quantity   
            from tbl_convert_product_out cpo
            join tbl_convert_master cpm on cpm.id = cpo.convert_id
            where cpo.status = 'a'
            and cpo.product_id = " . $data->productId . "
            and cpo.branch_id = " . $this->brunch . "
