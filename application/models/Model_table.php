...
class Model_Table extends CI_Model
{
    ...
    public function generateMaterialCode()
    {
        ...
        $lastProduct = $this->db->query("select * from tbl_materials order by material_id desc limit 1");
        if ($lastProduct->num_rows() != 0) {
            $newMaterialId = $lastProduct->row()->material_id + 1;
            $zeros = array('0', '00', '000', '0000');
            $materialCode = 'M' . (strlen($newMaterialId) > count($zeros) ? $newMaterialId : $zeros[count($zeros) - strlen($newMaterialId)] . $newMaterialId);
        }
    ...
    public function generateMaterialSaleCode(){
        $code = "MS-0001";
        
        $lastRow = $this->db->query("select * from tbl_material_sale order by sale_id desc limit 1");
        if($lastRow->num_rows() != 0){
            $newCode = $lastRow->row()->sale_id + 1;
            $zeros = array('0', '00', '000');
    ...
    public function generateConvertProductCode()
    {
        $code = "CP-0001";

        $lastRow = $this->db->query("select * from tbl_convert_master order by id desc limit 1");
        if ($lastRow->num_rows() != 0) {
            $newCode = $lastRow->row()->id + 1;
            $zeros = array('0', '00', '000');
            $code = 'CP-' . (strlen($newCode) > count($zeros) ? $newCode : $zeros[count($zeros) - strlen($newCode)] . $newCode);
        }
    ...
    public function getTransactionSummary($date = null)
    {
        $transactionSummary = $this->db->query("
            ...
            /* Received */
            (
                select ifnull(sum(ms.paid), 0) from tbl_material_sale ms
                where ms.branch_id= " . $this->session->userdata('BRANCHid') . "
                and ms.status = 'a'
                " . ($date == null ? "" : " and ms.sale_date < '$date'") . "
            ) as received_materialsales,
            (
            ...
            /* total */
            (
                select received_materialsales + received_sales + received_customer + received_supplier + received_cash + bank_withdraw + loan_received + loan_initial_balance + invest_received + sale_asset
            ) as total_in,
            (
                select paid_materialpurchase + paid_purchase + paid_customer + paid_supplier + paid_cash + bank_deposit + employee_payment + loan_payment + invest_payment + buy_asset
            ) as total_out,
            (
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    ci.*,
                    (select (ci.production_quantity + ci.purchase_quantity + ci.convert_quantity_in + ci.sales_return_quantity + ci.transfer_to_quantity) - (ci.sales_quantity + ci.convert_quantity_out + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
    ...
    public function supplierDue($clauses = "", $date = null)
    {
        ...
        $supplierDues = $this->db->query("
            ...
            s.contact_person,
            s.previous_due,
            (select ifnull(sum(mp.total), 0.00) from tbl_material_purchase mp
                where mp.Supplier_SlNo = s.Supplier_SlNo
                " . ($date == null ? "" : " and mp.purchase_date < '$date'") . "
                and mp.status = 'a'
            ) as materialBill,

            (select ifnull(sum(mp2.paid), 0.00) from tbl_material_purchase mp2
                where mp2.Supplier_SlNo = s.Supplier_SlNo
                " . ($date == null ? "" : " and mp2.purchase_date < '$date'") . "
                and mp2.status = 'a'
            ) as materialInvoicePaid,
             
            (select ifnull(sum(pm.PurchaseMaster_TotalAmount), 0.00) from tbl_purchasemaster pm
            ...
            (select bill + materialBill + cashReceived) as debit,

            (select invoicePaid + materialInvoicePaid + cashPaid + returned) as paid,
            
            (select (debit + ifnull(s.previous_due, 0)) - paid) as due
    ...
    public function customerDue($clauses = "", $date = null)
    {
        ...
        $dueResult = $this->db->query("
            ...
            c.owner_name,
            c.previous_due,
            (select ifnull(sum(ms.total), 0.00)
                from tbl_material_sale ms 
                where ms.Customer_SlNo = c.Customer_SlNo
                " . ($date == null ? "" : " and ms.sale_date < '$date'") . "
                and ms.status = 'a') as materialbillAmount,

            (select ifnull(sum(ms.paid), 0.00)
                from tbl_material_sale ms
                where ms.Customer_SlNo = c.Customer_SlNo
                " . ($date == null ? "" : " and ms.sale_date < '$date'") . "
                and ms.status = 'a') as materialinvoicePaid,
            ...
            (select billAmount + paidOutAmount + materialbillAmount) as debit,
            (select invoicePaid + cashReceived + returnedAmount + materialinvoicePaid) as paidAmount,

            (select (debit + ifnull(c.previous_due, 0.00)) - paidAmount) as dueAmount
    ...
    public function productStock($productId)
    {
        ...
        if ($stockCount != 0) {
            ...
            $stock = ($stockRow->production_quantity + $stockRow->convert_quantity_in + $stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity)
                - ($stockRow->convert_quantity_out + $stockRow->sales_quantity + $stockRow->purchase_return_quantity + $stockRow->damage_quantity + $stockRow->transfer_from_quantity);
        }
